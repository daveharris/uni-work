...
class TimeTable {
	...
	/* Your Code Here */
		timeTable = new Activity[DAYS][HOURS];							/*Creates  a new array, 'called timeTable' */
	}
	...
	/* Your Code Here */
		...
		int day = readDay("Enter the day");								/* Prompts the user for the day wanted*/
		int hour = readHour("Enter the time");							/* Promts the user for the time wanted */
		Activity temp = new Activity();									/* Creates a new activity object*/

		if (timeTable[day][hour] == null) {
			temp.prompt();												/* Calls on the prmpt method form the activity class*/
			timeTable[day][hour] = temp;								/* Puts the objects into the array*/
			printTimeTable();											/* prints the changes to the timetable*/
		}
			else {
	...
	/* Your Code Here */
		...
		if (timeTable[day][hour] == null) {
			...
			if (confirm == 0) {
				timeTable[day][hour] = null;							/* Sets the specified place in the array to null ie.deleting */
			}
				else {
					return;
				}
 ...
 /* save the timetable to a file */
	...
	/* Your Code Here */
		try {
			String outFileName = JOptionPane.showInputDialog("Output file name");	/* Asks the user for the name of the file to be saved*/
			FileWriter outStream = new FileWriter(outFileName);
			PrintWriter outs = new PrintWriter(outStream);				/* Sets up the file-writing and printing  */
