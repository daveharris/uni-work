...
/** Program:      Main Question for Comp102 -  Lab 8
  *  Author: David Harris
  *  Date: 14/05/03
  *  Description:  This program will allow the user to create and manipulate 
  * timetables. The program will first display an empty timetable and give the
  * userthe option to:
...
public class TimeTableProgramUI {
  public static void main(String[] args) {
    TimeTable tTable = new TimeTable();
    String[] menu =  {
      "New", "Load", "Add", "Delete", "Search", "Free Time", "Save", "Exit"
        }
    ;

    while (true) {
      int choice = JOptionPane.showOptionDialog(null, "select a function",
                                                "Menu", 
...
/** Class:       TimeTable
  * 
  *  Description: The TimeTable class represents a timetable.  The timetable 
  * is stored in a 2D array indexed by day and hour of an activity.
...
class TimeTable {
  ...
  /* Fields */
    ...
    /* add an activity to the timetable. Ask the user for day and time, then for
     * the activity, then add to the timetable
     */
    /* Your Code Here */
...
int confirm = JOptionPane.showConfirmDialog(null,"There is an activity there, continue?", "Confim", JOptionPane.YES_NO_OPTION);
  ...
  public void searchCourse() {
    ...
    String course =
        JOptionPane.showInputDialog("Enter the course eg.COMP102");
    for (int d = 0; d < DAYS; d++) {
      for (int h = 0; h < HOURS; h++) {
        if (timeTable[d][h] != null) {
          if ((timeTable[d][h].getCourse()).equals(course)) {
            System.out.print(course + "Found at ");
            printHour(h);
            System.out.print("on ");
            printDay(d);
            System.out.println();
  ...
  public void freeTime() {
    /* Your Code Here */
    String inputStr =
        JOptionPane.showInputDialog("Number of free hours to find");
  ...
  private int readDay(String str) {
    ...
    while (true) {
      ...
      d.toLowerCase();
      if ("monday ".startsWith(d))
        return 0;
      else if ("tuesday ".startsWith(d))
        return 1;
      else if ("wednesday ".startsWith(d))
        return 2;
      else if ("thursday ".startsWith(d))
        return 3;
      else if ("friday ".startsWith(d))
        return 4;
...
class Activity {
  ...
  /* Your Code Here */
  public void writeToFile(PrintWriter file)throws NumberFormatException,
      IOException {
  ...
  /* Your Code Here */
  public void readFromFile(BufferedReader file)throws NumberFormatException,
      IOException {
