...
public class MiniDraw implements ActionListener, MouseListener, MouseMotionListener{
	...
	public MiniDraw(){
		...
		//The buttons
		//Make the panels to add the buttosn to 

		JPanel buttonPanelTop = new JPanel();
		JPanel buttonPanelBott = new JPanel();

		// Initalize the buttons and make them listen to actions
		// Then add them to the panels
	...
	public void actionPerformed(ActionEvent e){
		...
		//Set the currentpoly to null, so that when you click
		//on another button, it does continue to add to line.
		currentPoly = null;
	...
	private void selectColor(){
		//Set up a dialog box to choose colour
	...
	public void mousePressed(MouseEvent e) {
		...
		lastX = e.getX();		//Get the x co-ord
		lastY = e.getY();		//get the y co-ord
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (brushShape.equals("Move")){
			//Calls the moveshape method
			moveShape(e.getX(), e.getY());
		}
		else if (brushShape.equals("Delete")){
			//Calls the deleteShape method
			deleteShape(e.getX(), e.getY());
		}
		else
			//If none of above to to drawShape 
	...
	public void mouseDragged(MouseEvent e) {
		...
		//MAkes the rubberebanding work, by inverting the color
	...
	private void invertRubberBand(int x, int y){
		//Sets up the rubberbanding for each individual shape
	...
	public void render(){
		//Go through the vector drawing each shape tot he canvas
		//Then displays it at the end, to stop flickering
		canvas.clear(false);
	...
	public void newDrawing(){
		//Creates a new vector, and thus deleting past elements 
		shapes = new Vector();
		render();
	}
	...
	public void openDrawing(){
		//Reads in the string from a file, and renders them to the canvas
		try {
			shapes = new Vector();
	...
	public void saveDrawing(){
		//Saves the shapes to a file in the form of a string, 
		//so can be read back in by OpenDrawing
		try {
	...
	*/
	public void deleteShape(int x, int y){
		...
		//Uses the pointOnShape method which returns a boolena value,
		// if true, the delete the shape from canvas and vector.
		for (int i=shapes.size()-1; i>=0; i--){
			Shape s = (Shape)shapes.elementAt(i);
	...
	public void drawShape(int x, int y){
		...
		Shape shape = null;
		//Calls the appropriate method below, according to waht shape was chosen 
		//by the button clicked.
		if (brushShape.equals("Line"))
	...
	public Shape newRectangle(int x, int y){
		//Make a new rectangle according to the rectangle class with
		//dimensions as recorded by mouse clicked and released
		int left= Math.min(lastX, x);
		int top= Math.min(lastY, y);
		int width= Math.abs(lastX - x);
		int height= Math.abs(lastY - y);
	...
	public Shape newOval(int x, int y){
		//Make a new oval according to the oval class with
		//dimensions as recorded by mouse clicked and released
		int left= Math.min(lastX, x);
		int top= Math.min(lastY, y);
		int width= Math.abs(lastX - x);
	...
	public Shape newPolyLine(int x, int y){
		//Make a new polyshape according to the polyLine class with
		//dimensions as recorded by mouse clicked and released
		if(currentPoly == null)
	...
	public Shape newTextShape(int x, int y){
		//Make a new text-box according to the textShape class with
		//positioned by mouse clicked
