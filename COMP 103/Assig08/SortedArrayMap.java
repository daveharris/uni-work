...
/**
	...
	* It keeps the items in order, determined by the compareTo method on the 
	* keys ie, keys must implement Comparable
	* null keys are not allowed, and cause a "NoSuchElementException"
	* It is efficient for accessing items; it is slower to insert.
	* When full (like Vector) it will create a new array of double the current 
	* size, andcopy all the items over to the new array
	*/
...
public class SortedArrayMap implements Map {
	...
	/** Remove a key (and any associated value)
		* Makes no change to the map if the key is not present.
	...
	private int binarySearch(Object key) {
		...
		while (low < high) {
			int mid = (low + high) / 2;
												// low < high, therefore mid >= low and mid < high
			if (k.compareTo(elementData[mid].key) > 0)
												// key should be after mid (ie [mid+1 .. high] )
