...
public class MiniDraw implements ActionListener, MouseListener,
	...
	private void selectColor() {
		...
		if (newColor != null) {
			...
			Action newAction = new Action(currentColor, "Color");
	...
	public void moveShape(int newX, int newY) {
		...
		for (int i = shapes.size() - 1; i >= 0; i--) {
			...
			if (s.pointOnShape(lastX, lastY)) {
				...
				Action newAction = new Action(brushShape, s,(newX - lastX),(newY - lastY));
				history.addLast(newAction);
				render();
				break;
			}
	...
	public void deleteShape(int x, int y) {
		...
		for (int i = shapes.size() - 1; i >= 0; i--) {
			...
			if (s.pointOnShape(x, y)) {
				...
				Action newAction = new Action(brushShape, i, s);
				history.addLast(newAction);
				render();
				break;
			}
	...
	public void drawShape(int x, int y) {
		...
		Action newAction = new Action(brushShape);
		history.addLast(newAction);
		canvas.display();
	}
	...
	private void undo () {
		if(!history.isEmpty()) {
			...
			String str = top.getActionType();
			if(str.equals("Line") || str.equals("Oval") || str.equals("Rect") ||
									 str.equals("Poly") || str.equals("Text"))
				shapes.removeElementAt(shapes.size()-1);
			else if(str.equals("Move")) {
				Shape last = (Shape)shapes.elementAt(shapes.size()-1);
				last.moveBy(-top.getMvx(), -top.getMvy());
				shapes.setElementAt(last, shapes.size()-1);
			}
			else if(str.equals("Delete"))
				shapes.addElementAt(top.getShape(), shapes.size());
			else if(str.equals("Color")) {
				Color oldColor = top.getColor();
